...
import { getOrCreateWeeklyRecord, saveLordsTableStats } from '@/app/leader/actions'
import Link from 'next/link'

export default function LordsTableForm({
  districtId,
  districtName,
  weekDate,
  existingAttendance,
  ...
  districtName: string
  weekDate: string
  existingAttendance: number | null
  existingNotes: string | null
}) {
  const [attendance, setAttendance] = useState(existingAttendance ?? 0)
  ...
  async function handleSave() {
    ...
    try {
      ...
      await saveLordsTableStats(record.id, attendance, notes || undefined)
      setSaved(true)
    } catch (e: any) {
  ...
  return (
    <div className="max-w-lg mx-auto">
      ...
      <h1 className="text-2xl font-bold text-gray-900 mb-2">Lord&apos;s Table Meeting</h1>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 space-y-5">
        <div>
          ...
          <p className="text-xs text-gray-400 mb-2">High schooler age and above</p>
          <input
            type="number"
            min={0}
            value={attendance}
